# -*- coding: utf-8 -*-
"""
Created on Mon May 24 16:59:28 2021
...
def show_result(G, x_, y_, num_epoch, show = False, save = False, path = 'result.png'):
    # G.eval()
    test_images = G(x_).squeeze(1)
    #print("shape of test images: ", test_images.size())
    ...
    for i in range(x_.size()[0]):
        ...
        ax[i, 1].cla()
        ax[i, 1].imshow((test_images[i].cpu().data.numpy() + 1) / 2, cmap = 'gray')
        ax[i, 2].cla()
        ax[i, 2].imshow((y_[i].numpy().transpose(1, 2, 0).squeeze(-1) + 1) / 2, cmap = 'gray')
...
def show_individual_result(G, x_, y_, show = False, save = False, path = 'result.png'):
    # G.eval()
    test_images = G(x_).squeeze(1)
    #print("shape of test images: ", test_images.size())
    ...
    if save:
      for i in range(x_.size()[0]):
          #ax[i, 0].cla()
          plt.imsave(path + 'RGB_' + str(i) + '.png',
                    x_[i].cpu().data.numpy().transpose(1, 2, 0))
          #ax[i, 1].cla()
          plt.imsave(path + 'Generated_IR_' + str(i) + '.png',
                    test_images[i].cpu().data.numpy(),
                    cmap = 'gray')
          #ax[i, 2].cla()
          plt.imsave(path + 'GT_IR_' + str(i) + '.png',
                    y_[i].numpy().transpose(1, 2, 0).squeeze(-1),
                    cmap = 'gray')
...
def average_psnr(G, x_, y_):
  test_images = G(x_).squeeze(1)
  psnr_list = [cv2.PSNR(y_[i].cpu().numpy().transpose(1, 2, 0).squeeze(-1),
                        test_images[i].cpu().data.numpy()) for i in range(x_.size()[0])]
  avg_psnr = sum(psnr_list) / float(len(psnr_list))
  return avg_psnr

def average_ssim(G, x_, y_):
  test_images = G(x_).squeeze(1)
  ssim_list = [ssim(y_[i].cpu().numpy().transpose(1, 2, 0).squeeze(-1),
                        test_images[i].cpu().data.numpy()) for i in range(x_.size()[0])]
  avg_ssim = sum(ssim_list) / float(len(ssim_list))
  return avg_ssim
